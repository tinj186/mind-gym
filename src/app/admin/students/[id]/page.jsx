...
export default async function StudentNeuralDashboard({ params }) {
  ...
  return (
    <div className="space-y-12 text-slate-100">
      {/* Header: Vital Signs */}
      <header className="flex justify-between items-end border-b-8 border-slate-700 pb-8">
        <div>
      ...
      <div className="grid grid-cols-12 gap-12">
        ...
        <section className="col-span-8 space-y-8">
          ...
          <div className="grid grid-cols-1 gap-6">
            {stats.mastery.map((m) => (
              <div key={m.id} className="p-8 bg-slate-700 border border-slate-600 rounded-[2rem] shadow-sm hover:-translate-y-1 transition-transform">
                ...
                <div className="flex items-center gap-8 w-1/2">
                  <div className="flex-1 h-6 bg-slate-800 rounded-full overflow-hidden border-2 border-slate-600 p-1">
                    <div 
                ...
                {m.fluencyMetrics && (
                  <div className="flex gap-4 mt-4 pt-4 border-t border-slate-200">
                    <div className="text-[10px] font-black uppercase text-slate-500">
                      CORRECTNESS <span className="text-white">{m.fluencyMetrics.correctness}%</span>
                    </div>
                    <div className="text-[10px] font-black uppercase text-slate-500">
                      EFFICIENCY <span className="text-white">{m.fluencyMetrics.efficiency}%</span>
                    </div>
                    <div className="text-[10px] font-black uppercase text-slate-500">
                      CONSISTENCY <span className="text-white">{m.fluencyMetrics.consistency}%</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        <section className="col-span-4 space-y-8">
          <div className="space-y-6">
            <h2 className="text-2xl font-black tracking-tight underline decoration-rose-500 decoration-8 underline-offset-4 text-white">BOTTLENECKS</h2>
            <div className="space-y-4">
              {stats.mastery.filter(m => m.synapseStrength < 40).map(b => (
                <div key={b.id} className="p-5 bg-rose-950/30 border-4 border-rose-900/50 rounded-[2rem] flex gap-4 items-center animate-pulse">
                  <span className="text-2xl">⚠️</span>
                  <div>
                    <p className="font-black text-rose-900 text-sm uppercase leading-tight">{b.subtopic}</p>
                    <p className="text-[10px] font-bold text-rose-400 uppercase">Form failure detected. Suggest remedial reps.</p>
                  </div>
                </div>
              ))}
              {stats.mastery.filter(m => m.synapseStrength < 40).length === 0 && (
                <p className="text-slate-400 italic text-sm text-center py-12 border-2 border-slate-700 rounded-3xl">No critical bottlenecks found.</p>
              )}
            </div>
          </div>

          <div className="space-y-6">
            <h2 className="text-2xl font-black tracking-tight text-white">RECENT LOGS</h2>
            <div className="space-y-3 font-mono text-[10px] bg-slate-800 p-6 rounded-3xl border-2 border-slate-700">
              {stats.recentLogs.map(log => (
                <div key={log.id} className="flex justify-between items-center border-b border-slate-700 pb-2 last:border-0 last:pb-0">
