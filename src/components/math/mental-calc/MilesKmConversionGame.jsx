...
export default function MilesKmConversionGame() {
  ...
  const [isMilesToKm, setIsMilesToKm] = useState(true);
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mentalCalcMilesKmScores');
    if (saved) {
  ...
  const generateQuestion = () => {
    const direction = Math.random() < 0.5; // true = miles -> km, false = km -> miles
    setIsMilesToKm(direction);
    
    // To keep the mental math clean for the trick, let's use multiples of 10 between 10 and 500
    const val = (Math.floor(Math.random() * 50) + 1) * 10;
    setNum(val);
    
    if (direction) {
      // Miles to Km
      // Trick: add half + add 10% = val + (val * 0.5) + (val * 0.1) = val * 1.6
      const est = val + (val * 0.5) + (val * 0.1);
      setExpectedEstimate(est);
      // Exact conversion: 1 mile = 1.60934 km
      setActualExact(val * 1.60934);
    } else {
      // Km to Miles
      // Trick: halve it + add 10% = (val * 0.5) + (val * 0.1) = val * 0.6
      const est = (val * 0.5) + (val * 0.1);
      setExpectedEstimate(est);
      // Exact conversion: 1 km = 0.621371 miles
      setActualExact(val * 0.621371);
    }
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      ...
      if (Math.abs(answer - expectedEstimate) < 0.01) {
        ...
        const qSymbol = isMilesToKm ? `${num} mi → km` : `${num} km → mi`;
  ...
  const finishGame = (finalSplits, totalDuration) => {
    ...
    if (!currentRecord || avgTime < currentRecord) {
      ...
      localStorage.setItem('mentalCalcMilesKmScores', JSON.stringify(newScores));
    }
  };
        ...
        <div className="text-center max-w-4xl">
          ...
          <div className="bg-slate-800/50 border border-slate-700 p-8 rounded-2xl mb-12 text-left">
            ...
            <div className="text-slate-300 space-y-6 font-medium leading-relaxed">
              <div>
                <h4 className="text-emerald-400 font-bold mb-2">Miles to Kilometers (Approx. x1.6)</h4>
                <p>1. Add half of the miles to the original miles.</p>
                <p>2. Then add 10% of the original miles.</p>
                <p className="text-sm text-slate-400 mt-1">Example: 40 miles &rarr; 40 + 20 + 4 = 64 km.</p>
              </div>
              
              <div className="border-t border-slate-700 pt-4">
                <h4 className="text-emerald-400 font-bold mb-2">Kilometers to Miles (Approx. x0.6)</h4>
                <p>1. Halve the kilometers.</p>
                <p>2. Then add 10% of the original kilometers.</p>
                <p className="text-sm text-slate-400 mt-1">Example: 40 km &rarr; 20 + 4 = 24 miles.</p>
              </div>
            </div>
          </div>
