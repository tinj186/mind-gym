...
const MULTIPLICATION_HINTS = {
  2: "just double it",
  3: "double it and add original number",
  4: "double it twice",
  5: "halve the number and add a zero",
  6: "multiply by 3 then double it / multiply by 6 / multiply by 5 and add original number",
  7: "normal multiplication",
  8: "double it three times / multiply by 10 then subtract double the original number",
  9: "triple number 2 times / multiply by 10 then subtract the original number",
  11: "adding up 2 digits next to each other, add 1st digit and last digit to number",
  12: "multiply by 3 then double 2 times / or multiply by 11 then add original number",
  15: "multiply by 3 then by 5 / add a zero to the number then add that number to half of itsself",
  16: "double the number 4 times",
  18: "multiply by 6 then by 3 / multiply by 20 then subtract 2 times the original number",
  21: "multiply by 20 then add of the original number",
  22: "multiply by 2 then multiply by 11"
};

export default function MultiplicationGame({ multiplier }) {
  ...
  const storageKey = `mentalCalcMult_${multiplier}_Scores`;
  ...
  const generateQuestion = () => {
    // Determine digit length randomly from 1 to 5
    ...
    else if (digits === 4) { min = 1000; max = 9999; }
    else { min = 10000; max = 99999; }
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      ...
      const correctAnswer = num1 * multiplier;
      
      if (answer === correctAnswer) {
        // Correct
        const now = Date.now();
        const splitDuration = now - lastSplitTime;
        const newSplits = [...splits, { q: `${num1.toLocaleString()} × ${multiplier} = ${answer.toLocaleString()}`, time: splitDuration }];
        setSplits(newSplits);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative">
        ...
        <div className="text-center max-w-4xl">
          ...
          <h1 className="text-5xl font-black mb-4 text-white">Multiply by {multiplier}</h1>
          <p className="text-slate-400 mb-8">Numbers from 1 to 5 digits. Press ENTER to submit your answer. The clock does not stop for mistakes.</p>
          
          <div className="bg-slate-800/50 p-6 rounded-3xl border-2 border-amber-500/30 text-amber-100 font-medium mb-12 max-w-2xl mx-auto shadow-lg shadow-amber-900/20">
            <span className="font-black text-amber-500 uppercase tracking-widest text-sm block mb-2">Strategy Hint</span>
            {MULTIPLICATION_HINTS[multiplier] || "Multiply accurately."}
          </div>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative" onClick={() => inputRef.current?.focus()}>
        ...
        <div className="flex flex-col items-center w-full max-w-5xl">
          <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 mb-12 w-full">
            <div className="text-[70px] md:text-[100px] font-black tracking-tighter tabular-nums text-white flex-shrink-0">
              {num1.toLocaleString()} <span className="text-slate-600">× {multiplier}</span>
            </div>
            <div className="text-[40px] md:text-[60px] font-black text-slate-500 flex-shrink-0 hidden md:block">
              =
            </div>
            <input
              ref={inputRef}
              ...
              className={`w-[320px] md:w-[480px] text-center text-[60px] md:text-[90px] font-black bg-slate-800 rounded-3xl outline-none border-4 transition-colors shadow-2xl ${
          ...
          <div className="mt-8 text-slate-600 font-medium">
            <span className="font-bold text-slate-500">Hint:</span> {MULTIPLICATION_HINTS[multiplier]}
          </div>
        </div>
      </div>
    );
          ...
          <div className="text-left mb-12">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {splits.map((split, idx) => (
                <div key={idx} className="bg-slate-900 p-4 rounded-2xl flex flex-col items-center border border-slate-800 text-center">
                  <div className="text-slate-500 font-bold mb-1 text-xs tracking-wider">#{idx + 1}</div>
                  <div className="text-white font-black text-lg mb-1 whitespace-nowrap overflow-hidden text-ellipsis w-full" title={split.q}>{split.q}</div>
