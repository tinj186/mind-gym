...
export default function SquaringGame({ digits = 2 }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`mentalCalcSquaring${digits}Scores`);
    if (saved) {
  ...
  const generateQuestion = () => {
    ...
    while (!valid) {
      if (digits === 2) {
        n = Math.floor(Math.random() * 89) + 11; // 11 to 99
      } else {
        n = Math.floor(Math.random() * 899) + 101; // 101 to 999
      }
      if (n % 10 !== 0) valid = true; // avoid trivial multiples of 10
    }
    setNum(n);
    setInputValue('');
    setIsError(false);
  ...
  const finishGame = (finalSplits, totalDuration) => {
    ...
    if (!currentRecord || avgTime < currentRecord) {
      ...
      localStorage.setItem(`mentalCalcSquaring${digits}Scores`, JSON.stringify(newScores));
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative">
        ...
        <div className="text-center max-w-3xl">
          ...
          <h1 className="text-5xl font-black mb-4 text-white">Squaring ({digits} Digits)</h1>
          <p className="text-slate-400 mb-8">Press ENTER to submit your answer. The clock does not stop for mistakes.</p>
          
          <div className="bg-slate-800/50 border border-slate-700 p-6 rounded-2xl mb-12 text-left">
            <h3 className="text-amber-500 font-black mb-3">💡 Pro Tip: Squaring Trick</h3>
            <ol className="list-decimal pl-5 text-slate-300 space-y-2 text-sm md:text-base font-medium leading-relaxed">
              <li>Round the number to the nearest multiple of 10 (up or down).</li>
              <li>Apply the difference to the original number but in the opposite direction.</li>
              <li>Do the multiplication of those two numbers instead of the original.</li>
              <li>Square the difference you took away and add that to the answer!</li>
            </ol>
          </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative" onClick={() => inputRef.current?.focus()}>
        ...
        <div className="flex flex-col items-center">
          <div className="text-[120px] md:text-[180px] font-black tracking-tighter mb-12 tabular-nums text-white flex items-start">
            {num}<span className="text-[60px] md:text-[90px] mt-4 ml-2 text-amber-500">2</span>
          </div>
          <input
            ref={inputRef}
            type="number"
