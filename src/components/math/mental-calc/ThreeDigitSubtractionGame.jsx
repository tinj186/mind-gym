...
export default function ThreeDigitSubtractionGame() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mentalCalcThreeDigitSubtractionScores');
    if (saved) {
  ...
  const generateQuestion = () => {
    ...
    setNum1(Math.max(a, b));
    setNum2(Math.min(a, b));
    setInputValue('');
    setIsError(false);
  };
  ...
  const finishGame = (finalSplits, totalDuration) => {
    ...
    if (!currentRecord || avgTime < currentRecord) {
      ...
      localStorage.setItem('mentalCalcThreeDigitSubtractionScores', JSON.stringify(newScores));
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative">
        ...
        <div className="text-center max-w-2xl">
          ...
          <h1 className="text-5xl font-black mb-4 text-white">Triple Digit Subtraction</h1>
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative" onClick={() => inputRef.current?.focus()}>
        ...
        <div className="flex flex-col items-center">
          <div className="text-[100px] md:text-[150px] font-black tracking-tighter mb-12 tabular-nums text-white">
            {num1} - {num2}
          </div>
          <input
            ref={inputRef}
