...
export default function ComplementsOneDigitGame() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mentalCalcComplements1DScores');
    if (saved) {
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      ...
      if (answer === target - num1) {
        ...
        const newSplits = [...splits, { q: `${num1} + ${answer} = 10`, time: splitDuration }];
        setSplits(newSplits);
  ...
  const finishGame = (finalSplits, totalDuration) => {
    ...
    if (!currentRecord || avgTime < currentRecord) {
      ...
      localStorage.setItem('mentalCalcComplements1DScores', JSON.stringify(newScores));
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative">
        ...
        <div className="text-center max-w-2xl">
          ...
          <h1 className="text-5xl font-black mb-4 text-white">Finding Complements (1 Digit)</h1>
          <p className="text-slate-400 mb-12">Find the number that adds up to 10. Press ENTER to submit your answer. The clock does not stop for mistakes.</p>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative" onClick={() => inputRef.current?.focus()}>
        ...
        <div className="flex flex-col items-center w-full max-w-2xl">
          <div className="flex items-center justify-center gap-8 mb-12 w-full">
            <div className="text-[120px] md:text-[180px] font-black tracking-tighter tabular-nums text-white flex-shrink-0">
              {num1}
            </div>
            <div className="text-[80px] md:text-[120px] font-black text-slate-500 flex-shrink-0">
              +
            </div>
            <input
              ref={inputRef}
              ...
              className={`w-[200px] md:w-[280px] text-center text-[80px] md:text-[120px] font-black bg-slate-800 rounded-3xl outline-none border-4 transition-colors shadow-2xl ${
