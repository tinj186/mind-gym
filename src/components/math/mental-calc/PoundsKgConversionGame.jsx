...
export default function PoundsKgConversionGame() {
  ...
  const [isLbsToKg, setIsLbsToKg] = useState(true);
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mentalCalcPoundsKgScores');
    if (saved) {
  ...
  const generateQuestion = () => {
    const direction = Math.random() < 0.5; // true = lbs -> kg, false = kg -> lbs
    setIsLbsToKg(direction);
    
    // Multiples of 10 for clean mental math
    const val = (Math.floor(Math.random() * 50) + 1) * 10;
    setNum(val);
    
    if (direction) {
      // Pounds to Kg
      // Trick: divide by 2, then subtract 10% of that answer
      const halved = val / 2;
      const est = halved - (halved * 0.1);
      setExpectedEstimate(est);
      // Exact conversion: 1 lb = 0.453592 kg
      setActualExact(val * 0.453592);
    } else {
      // Kg to Pounds
      // Trick: double it, then add 10% of that answer
      const doubled = val * 2;
      const est = doubled + (doubled * 0.1);
      setExpectedEstimate(est);
      // Exact conversion: 1 kg = 2.20462 lbs
      setActualExact(val * 2.20462);
    }
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      ...
      if (Math.abs(answer - expectedEstimate) < 0.01) {
        ...
        const qSymbol = isLbsToKg ? `${num} lbs → kg` : `${num} kg → lbs`;
  ...
  const finishGame = (finalSplits, totalDuration) => {
    ...
    if (!currentRecord || avgTime < currentRecord) {
      ...
      localStorage.setItem('mentalCalcPoundsKgScores', JSON.stringify(newScores));
    }
  };
        ...
        <div className="text-center max-w-4xl">
          ...
          <div className="bg-slate-800/50 border border-slate-700 p-8 rounded-2xl mb-12 text-left">
            ...
            <div className="text-slate-300 space-y-6 font-medium leading-relaxed">
              <div>
                <h4 className="text-emerald-400 font-bold mb-2">Pounds to Kilograms</h4>
                <p>1. Divide by 2.</p>
                <p>2. Then subtract 10% of that answer.</p>
                <p className="text-sm text-slate-400 mt-1">Example: 40 lbs → (40 ÷ 2) = 20 → 20 - 2 = 18 kg.</p>
              </div>
              
              <div className="border-t border-slate-700 pt-4">
                <h4 className="text-emerald-400 font-bold mb-2">Kilograms to Pounds</h4>
                <p>1. Double it (multiply by 2).</p>
                <p>2. Then add 10% of that answer.</p>
                <p className="text-sm text-slate-400 mt-1">Example: 40 kg → (40 × 2) = 80 → 80 + 8 = 88 lbs.</p>
              </div>
            </div>
          </div>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative" onClick={() => inputRef.current?.focus()}>
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="text-3xl font-bold text-slate-400 mb-12 uppercase tracking-widest">
            {isLbsToKg ? 'Pounds → Kilograms' : 'Kilograms → Pounds'}
          </div>
