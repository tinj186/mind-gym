...
export default function ComplementsFourFiveDigitGame() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mentalCalcComplements45DScores');
    if (saved) {
  ...
  const generateQuestion = () => {
    // 50% chance for 4-digit, 50% chance for 5-digit
    const isFourDigit = Math.random() < 0.5;
    
    if (isFourDigit) {
      setTarget(10000);
      setNum1(Math.floor(Math.random() * 8999) + 1000); // 1000 to 9998
    } else {
      setTarget(100000);
      setNum1(Math.floor(Math.random() * 89999) + 10000); // 10000 to 99998
    }
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      ...
      if (answer === target - num1) {
        // Correct
        const now = Date.now();
        const splitDuration = now - lastSplitTime;
        const newSplits = [...splits, { q: `${num1.toLocaleString()} + ${answer.toLocaleString()} = ${target.toLocaleString()}`, time: splitDuration }];
        setSplits(newSplits);
  ...
  const finishGame = (finalSplits, totalDuration) => {
    ...
    if (!currentRecord || avgTime < currentRecord) {
      ...
      localStorage.setItem('mentalCalcComplements45DScores', JSON.stringify(newScores));
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative">
        ...
        <div className="text-center max-w-2xl">
          ...
          <h1 className="text-5xl font-black mb-4 text-white">Finding Complements (4-5 Digits)</h1>
          <p className="text-slate-400 mb-12">Find the number that adds up to 10,000 or 100,000. Press ENTER to submit your answer. The clock does not stop for mistakes.</p>
        ...
        <div className="flex flex-col items-center w-full max-w-5xl">
          <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 mb-12 w-full">
            ...
            <input
              ...
              autoFocus
            />
            <div className="text-[40px] md:text-[60px] font-black text-slate-500 flex-shrink-0">
