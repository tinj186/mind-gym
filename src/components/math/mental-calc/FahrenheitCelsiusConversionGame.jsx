...
export default function FahrenheitCelsiusConversionGame() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mentalCalcFahrenheitCelsiusScores');
    if (saved) {
  ...
  const generateQuestion = () => {
    const direction = Math.random() < 0.5; // true = F -> C, false = C -> F
    setIsFToC(direction);
    
    // Even numbers between -10 and 100 for clean mental math
    const val = (Math.floor(Math.random() * 55) - 5) * 2;
    setNum(val);
    
    if (direction) {
      // Fahrenheit to Celsius
      // Trick: subtract by 30, then halve it
      const est = (val - 30) / 2;
      setExpectedEstimate(est);
      // Exact conversion: (F - 32) * 5/9
      setActualExact((val - 32) * 5 / 9);
    } else {
      // Celsius to Fahrenheit
      // Trick: double it, then add 30
      const est = (val * 2) + 30;
      setExpectedEstimate(est);
      // Exact conversion: (C * 9/5) + 32
      setActualExact((val * 9 / 5) + 32);
    }
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      ...
      if (Math.abs(answer - expectedEstimate) < 0.01) {
        ...
        const qSymbol = isFToC ? `${num} °F → °C` : `${num} °C → °F`;
        
        let errorPct = 0;
        if (actualExact !== 0) {
          errorPct = Math.abs((expectedEstimate - actualExact) / actualExact) * 100;
        }
  ...
  const finishGame = (finalSplits, totalDuration) => {
    ...
    if (!currentRecord || avgTime < currentRecord) {
      ...
      localStorage.setItem('mentalCalcFahrenheitCelsiusScores', JSON.stringify(newScores));
    }
  };
        ...
        <div className="text-center max-w-4xl">
          ...
          <div className="bg-slate-800/50 border border-slate-700 p-8 rounded-2xl mb-12 text-left">
            ...
            <div className="text-slate-300 space-y-6 font-medium leading-relaxed">
              <div>
                <h4 className="text-emerald-400 font-bold mb-2">Fahrenheit to Celsius</h4>
                <p>1. Subtract 30.</p>
                <p>2. Then halve it (divide by 2).</p>
                <p className="text-sm text-slate-400 mt-1">Example: 80 °F → (80 - 30) = 50 → 50 ÷ 2 = 25 °C.</p>
              </div>
              
              <div className="border-t border-slate-700 pt-4">
                <h4 className="text-emerald-400 font-bold mb-2">Celsius to Fahrenheit</h4>
                <p>1. Double it (multiply by 2).</p>
                <p>2. Then add 30.</p>
                <p className="text-sm text-slate-400 mt-1">Example: 25 °C → (25 × 2) = 50 → 50 + 30 = 80 °F.</p>
              </div>
            </div>
          </div>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative" onClick={() => inputRef.current?.focus()}>
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="text-3xl font-bold text-slate-400 mb-12 uppercase tracking-widest">
            {isFToC ? 'Fahrenheit → Celsius' : 'Celsius → Fahrenheit'}
          </div>
                  ...
                  {splits.map((split, i) => (
                    <tr key={i} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="p-4 tabular-nums font-bold">
                        <span className={split.err > 5 ? 'text-red-400' : 'text-amber-400'}>
                          {split.err.toFixed(1)}%
                        </span>
                      </td>
