...
export default function ComplementsTwoThreeDigitGame() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mentalCalcComplements23DScores');
    if (saved) {
  ...
  const generateQuestion = () => {
    // 50% chance for 2-digit, 50% chance for 3-digit
    const isTwoDigit = Math.random() < 0.5;
    
    if (isTwoDigit) {
      setTarget(100);
      setNum1(Math.floor(Math.random() * 89) + 10); // 10 to 98
    } else {
      setTarget(1000);
      setNum1(Math.floor(Math.random() * 899) + 100); // 100 to 998
    }
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      ...
      if (answer === target - num1) {
        ...
        const newSplits = [...splits, { q: `${num1} + ${answer} = ${target}`, time: splitDuration }];
        setSplits(newSplits);
  ...
  const finishGame = (finalSplits, totalDuration) => {
    ...
    if (!currentRecord || avgTime < currentRecord) {
      ...
      localStorage.setItem('mentalCalcComplements23DScores', JSON.stringify(newScores));
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative">
        ...
        <div className="text-center max-w-2xl">
          ...
          <h1 className="text-5xl font-black mb-4 text-white">Finding Complements (2-3 Digits)</h1>
          <p className="text-slate-400 mb-12">Find the number that adds up to 100 or 1,000. Press ENTER to submit your answer. The clock does not stop for mistakes.</p>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-6 relative" onClick={() => inputRef.current?.focus()}>
        ...
        <div className="flex flex-col items-center w-full max-w-4xl">
          <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 mb-12 w-full">
            <div className="text-[100px] md:text-[140px] font-black tracking-tighter tabular-nums text-white flex-shrink-0">
              {num1}
            </div>
            <div className="text-[60px] md:text-[100px] font-black text-slate-500 flex-shrink-0 hidden md:block">
              +
            </div>
            <input
              ref={inputRef}
              ...
              className={`w-64 md:w-[360px] text-center text-[80px] md:text-[120px] font-black bg-slate-800 rounded-3xl outline-none border-4 transition-colors shadow-2xl ${
                isError ? 'border-red-500 text-red-500' : 'border-slate-600 focus:border-amber-500 text-white'
              }`}
              autoFocus
            />
            <div className="text-[60px] md:text-[100px] font-black text-slate-500 flex-shrink-0">
