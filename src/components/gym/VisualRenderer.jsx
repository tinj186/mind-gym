...
export default function VisualRenderer({ type, data, visualProps, setIsToolOpen, questionId, difficulty, topic, attempts, isExam = false }) {
  console.log(`🎨 [VisualRenderer] Incoming Visual Type: "${type}"`, "Raw Data Payload:", data);
  ...
  switch (type) {
    ...
    case 'SINGAPORE_MONEY': {
      // Step 1 Troubleshooting: Mirror the exact robust data read array from NUMBER_CARDS
      const moneyItems = data?.items || data?.numbers || [];

      if (moneyItems.length === 0) {
        return (
          <div className="p-6 bg-amber-50 border-2 border-dashed border-amber-200 rounded-2xl text-center">
            <p className="text-amber-700 text-xs font-black uppercase">⚠️ Debugger: No Money Items Found in Payload</p>
          </div>
        );
      }
      ...
      return (
        <div className="flex flex-wrap justify-center gap-4 py-4 bg-slate-50 border-4 border-dashed border-slate-200 rounded-[2rem] p-6">
          {moneyItems.map((val, idx) => (
            <div 
              key={idx} 
              className="w-20 h-28 bg-white rounded-2xl border-4 border-indigo-200 flex flex-col items-center justify-center shadow-lg transform rotate-[-1deg] odd:rotate-[1deg]"
            >
              {/* Output the raw string received from the database directly */}
              <span className="text-xl font-black text-slate-900">{val?.toString()}</span>
              <span className="text-[8px] font-bold text-indigo-400 mt-1 uppercase tracking-tighter">Raw Token</span>
            </div>
          ))}
        </div>
      );
