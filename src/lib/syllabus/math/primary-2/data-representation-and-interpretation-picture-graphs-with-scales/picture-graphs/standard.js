...
export function standardLogic(activeVariant, difficulty, type, isMCQ, isShort, isStructure, zodType, zodDiff, levelName, topic, getFormatInstructions, context, selectedContextItem, getQText) {
  ...
  // Ensure we have a unique maximum and minimum for some variants
  ...
  if (activeVariant === 'standard_difference_between_categories') {
    ...
    if (isReverse) {
      ...
      shortText = `How many ${comparisonWord} ${theme.name.toLowerCase()} are there for ${subject.label} than for ${target.label}?`;
      structureText = `Look at the picture graph. How many ${comparisonWord} ${theme.name.toLowerCase()} are there for ${subject.label} than for ${target.label}? Show your working and the final answer.`;
