export function foundationLogic(activeVariant, difficulty, type, isMCQ, isShort, isStructure, zodType, zodDiff, levelName, topic, getFormatInstructions, context, selectedContextItem, getQText) {
  ...
  if (activeVariant === 'foundation_compare_greater_hundreds') {
    const h1 = Math.floor(Math.random() * 9) + 1;
    let h2 = Math.floor(Math.random() * 9) + 1;
    while (h2 === h1) {
      h2 = Math.floor(Math.random() * 9) + 1;
    }
    num1 = h1 * 100 + Math.floor(Math.random() * 100);
    num2 = h2 * 100 + Math.floor(Math.random() * 100);
    askType = "greater";
    ...
    num2 = h2 * 100 + Math.floor(Math.random() * 100);
    askType = "smaller";
  } else if (activeVariant === 'foundation_compare_greater_tens') {
    ...
    num1 = h * 100 + t1 * 10 + Math.floor(Math.random() * 10);
    num2 = h * 100 + t2 * 10 + Math.floor(Math.random() * 10);
    askType = "greater";
  } else if (activeVariant === 'foundation_compare_smaller_tens') {
    const h = Math.floor(Math.random() * 9) + 1;
    const t1 = Math.floor(Math.random() * 10);
    let t2 = Math.floor(Math.random() * 10);
    while (t2 === t1) {
    ...
    num2 = h * 100 + t2 * 10 + Math.floor(Math.random() * 10);
    askType = "smaller";
  } else if (activeVariant === 'foundation_compare_ones') {
    const h = Math.floor(Math.random() * 9) + 1;
