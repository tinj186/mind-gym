...
export const advancedVariants = {
  advanced_container: (config, type, isMCQ, isShort, isStructure, zodType, zodDiff, level, topic, formatInstructions, context, getQText) => {
    ...
    const storyInstruction = `CREATIVE INSTRUCTIONS:
      ${isShort ? "- Write a short, direct math question using the name ${sName}." : "- Write an engaging 1-sentence Singaporean math story context using the name ${sName}."}
      - CRITICAL: You MUST explicitly ask a question ending with '?' (Do not just write a statement).
      ${!isShort ? '- STRICT: Replace the "[STORY]" placeholder with your story context. DO NOT leave the "[STORY]" tag in your response.' : ''}`;
  ...
  advanced_comparison: (config, type, isMCQ, isShort, isStructure, zodType, zodDiff, level, topic, formatInstructions, context, getQText) => {
    ...
    let options = [answer, ORDINAL_SYMBOLS[Math.max(0, targetPos - 2)], ORDINAL_SYMBOLS[Math.min(9, targetPos)], ORDINAL_SYMBOLS[Math.min(9, targetPos + 1)]];
    options = [...new Set(options)];
    while(options.length < 4) {
    ...
    if (type === 'MCQ') {
      ...
      let defectMapObj = {
        ...
        [ORDINAL_SYMBOLS[Math.min(9, targetPos)]]: "CONCEPTUAL_ERROR",
        [ORDINAL_SYMBOLS[Math.min(9, targetPos + 1)]]: "CARELESS_CALCULATION"
      };
  ...
  advanced_multiple_leaves: (config, type, isMCQ, isShort, isStructure, zodType, zodDiff, level, topic, formatInstructions, context, getQText) => {
    ...
    let options = [answer, ORDINAL_WORDS[Math.max(0, newPos - 2)], ORDINAL_WORDS[Math.min(9, newPos)], ORDINAL_WORDS[Math.max(0, startPos - 1)]];
    options = [...new Set(options)];
    while(options.length < 4) {
    ...
    if (type === 'MCQ') {
      ...
      mcqOptions = JSON.stringify(options);
      let defectMapObj = {
        [ORDINAL_WORDS[Math.min(9, newPos)]]: "CONCEPTUAL_ERROR",
  ...
  advanced_net_queue_change: (config, type, isMCQ, isShort, isStructure, zodType, zodDiff, level, topic, formatInstructions, context, getQText) => {
    ...
    let options = [answer, ORDINAL_WORDS[Math.max(0, startPos - 2)], ORDINAL_WORDS[startPos - 1], ORDINAL_WORDS[Math.min(9, newPos)]];
    options = [...new Set(options)];
    while(options.length < 4) {
      options.push(ORDINAL_WORDS[Math.floor(Math.random() * 10)]);
      options = [...new Set(options)];
