...
export const standardVariants = {
  standard_reverse: (config, type, isMCQ, isShort, isStructure, zodType, zodDiff, level, topic, formatInstructions, context, getQText) => {
    ...
    let options = [backOrdinal, ORDINAL_SYMBOLS[Math.max(0, targetFromBack - 2)], ORDINAL_SYMBOLS[Math.min(9, targetFromBack)], ORDINAL_SYMBOLS[Math.min(9, targetFromBack + 1)]];
    ...
    if (type === 'MCQ') {
      ...
      let defectMapObj = {
        ...
        [ORDINAL_SYMBOLS[Math.min(9, targetFromBack)]]: "CONCEPTUAL_ERROR",
        [ORDINAL_SYMBOLS[Math.min(9, targetFromBack + 1)]]: "CARELESS_CALCULATION"
      };
  ...
  standard_from_the_right: (config, type, isMCQ, isShort, isStructure, zodType, zodDiff, level, topic, formatInstructions, context, getQText) => {
    ...
    let options = [answer, ORDINAL_WORDS[Math.max(0, rightPositionIndex - 1)], ORDINAL_WORDS[Math.min(9, rightPositionIndex + 1)], ORDINAL_WORDS[Math.min(9, rightPositionIndex + 2)]];
    options = [...new Set(options)];
    while(options.length < 4) {
    ...
    if (type === 'MCQ') {
      ...
      let defectMapObj = {
        ...
        [ORDINAL_WORDS[Math.min(9, rightPositionIndex + 1)]]: "CONCEPTUAL_ERROR"
      };
      options.forEach(opt => { if (opt !== answer && !defectMapObj[opt]) defectMapObj[opt] = "CARELESS_CALCULATION"; });
      defectMapStr = JSON.stringify(defectMapObj);
    }
  ...
  standard_relative_behind: (config, type, isMCQ, isShort, isStructure, zodType, zodDiff, level, topic, formatInstructions, context, getQText) => {
    ...
    let options = [answer, ORDINAL_WORDS[Math.max(0, targetPos - 3)], ORDINAL_WORDS[Math.max(0, targetPos - 2)], ORDINAL_WORDS[Math.min(9, targetPos)]];
    options = [...new Set(options)];
    while(options.length < 4) {
      options.push(ORDINAL_WORDS[Math.floor(Math.random() * 10)]);
      options = [...new Set(options)];
    ...
    if (type === 'MCQ') {
      ...
      let defectMapObj = {
        [ORDINAL_WORDS[Math.max(0, targetPos - 2)]]: "CONCEPTUAL_ERROR",
        [ORDINAL_WORDS[Math.min(9, targetPos)]]: "CONCEPTUAL_ERROR"
      };
      options.forEach(opt => { if (opt !== answer && !defectMapObj[opt]) defectMapObj[opt] = "CARELESS_CALCULATION"; });
      defectMapStr = JSON.stringify(defectMapObj);
    }
